...
## Round 173 — 2026-08-13 (audit round 1: outbound-email abuse guard + repo hygiene)

**Reviewer P1 (cost abuse):** the login-code endpoint only rate-limited per email (3/10 min), so enumerating fresh addresses could burn the whole Resend daily quota and hurt sender reputation. Systemic fix instead of point patches: all three Resend calls (login code, subscribe confirm, welcome) now go through a single `deliver()` gate in `src/auth.js` enforcing a per-IP hourly limit (10, keyed on `cf-connecting-ip`) plus a global daily circuit breaker (90 sends/day → callers see the existing 'quota' path with its honest user-facing copy). The login code is only written to KV after a successful send. Duplicated fetch/header code across the three senders was deleted in the process (fewer entities, one choke point).
**Repo hygiene (reviewer architecture note):** 27 process artifacts (`test-plan-iter*.md`, `test-report-*.md`) moved from the repository root into `docs/qa/`.
**Verified:** tests pass locally; deployed; production login send still works and repeated sends from one IP hit the new limit.

## Round 174 — 2026-08-14 (audit round 2: login resend cooldown, post-login return, empty-list dedupe)

**Reviewer P2 (empty-list toolbar density):** the empty grocery-list card duplicated the toolbar's staples entry ("Set up staples" next to "+ Add staples" on the same screen). The duplicate card button was removed — one entity fewer, toolbar stays the single home for staples.
**Reviewer suggestions adopted:** (1) the code-entry page now has a "Didn't get it? Resend code" form; plain HTML works without JS, and `app.js` adds a 60 s cooldown countdown so users don't burn the 3-sends/10-min allowance blind. (2) Logging in from a deep link now returns to it: `requireHousehold` redirects to `/login?next=<path>`, the value is carried through both login forms and validated by `safeNext()` (internal `/app…` paths only) before the post-verify redirect.
**Verified:** tests pass locally; deployed; production checks of resend control, next-param round-trip and empty-list card.

## Round 175 — 2026-08-14 (audit round 3: WCAG contrast fixed at the token layer)

**Reviewer P1 (axe serious color-contrast, landing ×3 + login/app ×25):** root-caused to the design-token layer, not individual nodes. Three systemic causes: (1) Tailwind v4's default `emerald-600` (#009966) gives only 3.65:1 under white button text; (2) our warm `stone-400` (#aaa090) gave 2.4:1 as muted text on cream; (3) past-day planner cards were dimmed with `opacity-60`, multiplying every inner text below threshold.
...
## Round 177 — 2026-08-14 (audit round 6: IndexNow cron + unified QA-traffic marking)

**IndexNow cron (org-wide requirement, ShelfMark pattern reused):** `runIndexNow(env)` pushes the full sitemap (39 URLs, batched at 8000 like ShelfMark) to api.indexnow.org from a weekly cron (`0 9 * * 1`); the sitemap URL list was extracted into `sitePaths()` shared by `/sitemap.xml` and the push. Cron registered in production (wrangler's routes-permission error aborts the triggers step, so the schedule was PUT via the API directly — a one-off full push returned 200 for all 39 URLs).
**Unified QA-traffic marking:** the analytics middleware skips counting (`analytics_daily` + `referrers_daily`) when the request carries UA substring `DevinQA`, header `x-qa-traffic`, or cookie `ml_qa=1`. Production-verified: 4 marked hits on /terms counted 0, 1 unmarked hit counted 1. Testing skill updated so all future QA sessions mark their traffic.
**Verified:** tests 29/29; `wrangler dev --test-scheduled` ran the scheduled handler cleanly; deployed.

## Round 178 — 2026-08-14 (audit round 7: AI endpoint cost gate + email-gate copy + horizontal matrix)

**P1 (AI endpoint unlimited):** `/app/ai/generate` now has daily KV cost caps reusing the auth.js gate pattern — per-household 10/day (`rl:ai:<day>:<hid>`) and per-IP 20/day. Hitting the cap redirects to `?ai=limit` with an amber alert and a "Fill from recipe box (no AI)" fallback; the AI button tooltip states "Up to 10 AI drafts a day."
**P2 (misleading limit copy):** `deliver()`/`sendMagicCode` now return `'limit'` distinctly from `'fail'`; /login shows "…the limit clears within an hour." Also, `sends:<email>` now increments only after a successful provider send, so a shared-IP block no longer burns the address's own allowance (QA finding this round).
**Beacon/analytics hardening (minimal, per report):** analytics middleware also skips non-browser UAs (curl/bot/spider/headless etc.), complementing the R177 QA marking.
**Horizontal matrix self-check (MealLoop × report matrix):** AI endpoint — now gated; email endpoints — gated since R173 (per-IP 10/h + global 90/day + per-email 3/send-success); admin endpoints (`/ops/stats|migrate|cleanup-qa`) — Bearer key, fail-closed 404 when unset; CSP — full policy incl. default-src/script-src 'self', no unsafe-inline; rate-limit key dimensions — email gates are per-IP+per-email+global, AI gate per-household+per-IP (no password login → no counter-reset issue); beacon forgery — server-side middleware only counts marked-clean browser HTML GETs, QA convention + bot filter + honest export labels.
**Verified:** tests 29/29; production run by testing agent: simulated cap → ai=limit banner passed, tooltip passed, 4th-code limit copy passed, baseline intact, QA account deleted. Relay flaked (ai=err) on the success-path attempt — limiter never false-positived; success smoke owed when relay recovers.

## Round 179 — 2026-08-14 (audit round 8: edge cache for public pages + three-layer rate limits)

**P1 (edge cache absent, systemic org-wide):** Workers never edge-cache fetch-handler responses, so `s-maxage` headers alone did nothing. Adopted NameChart's ~20-line `caches.default` middleware: public marketing pages (`sitePaths()` + robots/sitemap) are cached at the edge keyed by `path?edge=<ASSET_V>` with a 1 h `s-maxage` TTL. Session-scoped requests (`ml_session` cookie), non-GET, query-string URLs and `Set-Cookie` responses are never cached. Security headers and analytics counting still run per-request (middleware order: cache sits inside the header/analytics wrapper), so cached hits keep the full CSP and are still counted. `X-Edge-Cache: HIT/MISS` header for observability.
