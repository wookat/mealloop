...
4. **Counts are not forgery-proof.** Server-side view counting filters QA markers and obvious non-browser agents (bot/curl/headless UAs), but any client that mimics a browser can still inflate counts — there is deliberately no signed beacon. Treat page-view numbers as directional signals, never as hard business metrics.
5. **Interpretation.** With ~2 organic search referrals and 4 sign-ups in 30 days, the funnel is too thin for rate conclusions; the actionable signal is acquisition (indexing/distribution), not in-product retention. Bing Webmaster/GSC registration (audit round 5 suggestion) would add real impression/click data upstream of step 1.
