...
- Since Round 11, households have a display-only units preference ("Units: …" select in the /app/list action row, data-autosubmit → POST /app/settings/units). convertUnits() applies on list labels, recipe detail, and share pages (which follow the household setting, no viewer toggle, and auto-update via the sync poll after bumpVersion). Imperial: g→oz (lb when ≥454g), ml→fl oz; metric: oz→g, lb→g/kg; cups/tbsp/counts/kg-as-parsed-to-g and composite labels ("2 x 400g cans …") pass through. Stored labels never mutate — switching back to "as written" must restore originals exactly.
